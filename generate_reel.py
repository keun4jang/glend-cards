...
# 3) 릴스 대본 — 10개 장면(후킹+본문9). 정보 밀도 높은 85~90초 목표.
AVOID = avoid_line()
...
이 중에서 대중이 가장 반응할 핵심 주제 하나를 직접 골라서, 정보가 꽉 찬 세로 릴스 대본을 만들어줘. 시청자가 이 영상 하나만 보면 그 주제를 완전히 이해하고 바로 행동할 수 있어야 해. 전체 길이는 약 85~90초.
릴스는 10개 장면(scene)으로 구성돼. 각 장면은 성우가 말하는 동시에 화면 중앙에 그대로 뜨는 자막 문장(narration)과 배경 사진 검색어(query)로 이뤄져.
{AVOID}
...
- scene 2~10 = 본문 9개. 아래 흐름을 따라 각 장면이 서로 다른 알맹이를 담아:
  (2) 무슨 일인지 배경·맥락  (3) 핵심 내용과 정확한 수치  (4) 왜 이렇게 됐는지 이유
  (5) 누가 대상인지 조건  (6) 얼마를 받거나 아끼는지 구체적 금액  (7) 신청·실행 방법과 절차
  (8) 기한과 놓치기 쉬운 주의점  (9) 사람들이 흔히 하는 실수 또는 추가 꿀팁  (10) 정리와 행동 촉구
- 절대 같은 말을 다르게 반복하지 마. 각 장면은 앞에서 안 나온 새로운 정보를 하나씩 추가해야 해. 내용이 부족하면 그 주제를 고르지 말고 정보가 풍부한 다른 주제를 골라.
- narration은 성우가 소리 내어 읽는 문장이니, 실제 사람이 말하듯 자연스러운 구어체로 써. 친근한 '해요체'를 기본으로 하고(예: "~있어요", "~된대요", "~챙기세요"), 딱딱한 문어체나 어색한 번역투(예: "~을 통해", "~에 의해", "~라 할 수 있다")는 쓰지 마. 각 narration은 공백 포함 40~65자 정도로 충실하게 — 한 문장 또는 짧은 두 문장. 실제 수치·기관명·날짜·조건을 구체적으로 넣어 알맹이를 채워.
...
- scene 9나 10 중 하나의 narration 끝에 "저장해두고 다시 보세요" 같은 저장 유도를 자연스럽게 한 번 넣어. (저장 유도는 전체에서 딱 한 번만)
...
- 썸네일 커버(cover)도 만들어줘 — 피드에서 클릭을 부르는 카드뉴스식 후킹:
  ...
  - cover.sub = 한 줄 부제 (공백 포함 10~16자), 핵심 명사 1개만 <b>단어</b>로 강조 (동사·부사 강조 금지)
...
{{
  ...
  "cover": {{ "title": "1줄\\n2줄(한 줄 6자 이내)", "sub": "부제 한 줄(<b>포인트</b> 포함)" }},
  "scenes": [
    ...
    {{ "narration": "본문9 흔한 실수 또는 꿀팁", "query": "영어 사진 검색어" }},
    {{ "narration": "본문10 정리·행동 촉구", "query": "영어 사진 검색어" }}
  ],
...
def validate(d):
    ...
    scenes = d.get("scenes")
    assert isinstance(scenes, list) and len(scenes) == 10, f"scenes 개수 오류({len(scenes) if isinstance(scenes, list) else '없음'})"
...
FOLLOW_NARRATION = "이런 정보 매일 받아보고 싶다면, 지금 <b>팔로우</b> 눌러주세요!"
...
if True:
    ...
    # 로고 직전 팔로우 유도 장면 (첫 장면 배경 재사용 — 수미상관)
    n = len(data["scenes"])
    data["scenes"].append({
        "narration": FOLLOW_NARRATION,
        "query": "follow cta",
        "bg": data["scenes"][0]["bg"],
    })
    print(f"  scene{n+1}: 팔로우 유도 (자막 있음)")

    # 마지막 아웃트로 (검정 배경 + 로고, 나레이션 없이 음악만)
    data["scenes"].append({"outro": True, "silent": True, "narration": ""})
    print(f"  scene{n+2}(아웃트로): 로고 + 음악")
