...
이 중에서 대중이 가장 반응할 핵심 주제 하나를 직접 골라서, 정보가 알찬 세로 릴스 대본을 만들어줘.
릴스는 6개 장면(scene)으로 구성돼. 각 장면은 성우가 말하는 동시에 화면 중앙에 그대로 뜨는 자막 문장(narration)과 배경 사진 검색어(query)로 이뤄져.

규칙:
- scene 1 = 후킹. narration은 시선을 확 잡는 짧고 강한 한 문장. {HOOK}
- scene 2~6 = 핵심 내용. 주제를 단계적으로 깊이 있게 풀어줘. 조건/대상/금액/기한/방법/주의점 등 구체적이고 실용적인 정보를 충실히 담아. 각 장면이 서로 다른 알맹이를 전달하도록.
- narration은 자연스러운 구어체. 후킹(scene1)은 짧게, 핵심 장면들(scene2~6)은 한두 문장으로 정보를 구체적으로(공백 포함 대략 25~55자). 내용이 풍부하고 자세할수록 좋아.
- 각 narration에서 가장 중요한 핵심 단어 1개만 <b>단어</b>로 감싸 강조(노란색). 장면당 1개만.
- 자막에 글자수 메모("(16자)" 등)를 절대 쓰지 마. 최종 문장만.
- query는 각 장면 분위기에 맞는 영어 사진 검색어 2~3단어 (예: {QEX}).
- 인스타 캡션: 첫 줄 후킹 + 핵심 4~5줄 + 저장/팔로우 유도 + 해시태그 5개.
